...
void main() async {
  ...
  group("channel", () {
    ...
    test("channel", () async {
      ...
      final controller = await client.streamingService.stream();
      final listener = controller.channelStream(channelId: channel.id).listen((
        event,
      ) {
        final body = event.body;
        if (body is NoteChannelEvent) completer.complete(body.body);
      ...
      await client.notes.create(
        NotesCreateRequest(text: "test", channelId: channel.id),
      );
      await completer.future;
    ...
    group("userList", () {
      test("note", () async {
        final completer = Completer<Note>();
        final client = userClient;
        final list = await client.users.list.create(
        ...
        final controller = await client.streamingService.stream();
        final listener = controller.userListStream(listId: list.id).listen((
          event,
        ) {
          final body = event.body;
          if (body is NoteChannelEvent) completer.complete(body.body);
    ...
    group("antenna", () {
      test("note", () async {
        ...
        final controller = await client.streamingService.stream();
        final listener = controller.antennaStream(antennaId: antenna.id).listen(
          (event) {
            final body = event.body;
    ...
    group("queueStats", () {
      test("statsLog", () async {
        ...
        final controller = await client.streamingService.stream();
        final id = DateTime.now().toIso8601String();
        final listener = controller.queueStatsLogStream(id: id).listen((event) {
          final body = event.body;
    ...
    group("chatRoom", () {
      ...
      test("react", () async {
        ...
        await client.chat.rooms.invitations.create(
          ChatRoomsInvitationsCreateRequest(roomId: room.id, userId: admin.id),
        );
    ...
    group("main", () {
      ...
      test("mention", () async {
        ...
        await adminClient.notes.create(
          NotesCreateRequest(text: "@${user.username}"),
        );
        await completer.future;
        await (controller.removeChannel(id), listener.cancel()).wait;
      });
      ...
      test("followed", () async {
        ...
        await newClient.following.create(
          FollowingCreateRequest(userId: user.id),
        );
        await completer.future;
        await (controller.removeChannel(id), listener.cancel()).wait;
      });
      ...
      test("receiveFollowRequest", () async {
        final completer = Completer<UserLite>();
        final newClient = (await adminClient.createUser()).client;
        final newUser = await newClient.i.update(
          IUpdateRequest(isLocked: true),
        );
      ...
      test("readAllAnnouncements", () async {
        ...
        final announcements = await newClient.announcements(
          AnnouncementsRequest(limit: 100),
        );
  ...
  group("noteUpdated", () {
    ...
    test("unreacted", () async {
      ...
      await client.notes.reactions.delete(
    ...
    test("pollVoted", () async {
      ...
      controller.subNote(note.id);
      await adminClient.notes.polls.vote(
  ...
  group("broadcast", () {
    test("emojiCreated", () async {
      ...
      await adminClient.apiService.post("admin/emoji/add", {
        "name": name,
        "fileId": file.id,
      });
      await completer.future;
    ...
    test("emojiUpdated", () async {
      ...
      final file = await adminClient.createImageDriveFile();
      final name = Uuid().v4().replaceAll("-", "_");
      final response = await adminClient.apiService.post("admin/emoji/add", {
        "name": name,
        "fileId": file.id,
      });
