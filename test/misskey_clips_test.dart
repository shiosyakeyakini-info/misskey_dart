...
void main() async {
  ...
  test("notes", () async {
    ...
    final response = await userClient.clips.notes(
      ClipsNotesRequest(clipId: clip.id),
    );
  ...
  test("remove-note", () async {
    ...
    final notes = await userClient.clips.notes(
      ClipsNotesRequest(clipId: clip.id),
    );
  ...
  test("update", () async {
    final clip = await userClient.clips.create(
      ClipsCreateRequest(name: "test", isPublic: false, description: "test"),
    );
    final updated = await userClient.clips.update(
      ClipsUpdateRequest(clipId: clip.id, name: "updated", isPublic: true),
    );
  ...
  test("show", () async {
    ...
    final response = await userClient.clips.show(
      ClipsShowRequest(clipId: clip.id),
    );
