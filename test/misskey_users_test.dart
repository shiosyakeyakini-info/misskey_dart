...
void main() async {
  ...
  group("show", () {
    test("me", () async {
      final response = await userClient.users.show(
    ...
    test("not me", () async {
      final response = await userClient.users.show(
        UsersShowRequest(userId: admin.id),
      );
    ...
    test("anonymous", () async {
      final response = await anonymousClient.users.show(
        UsersShowRequest(userId: user.id),
      );
  ...
  test("showByIds", () async {
    final response = await userClient.users.showByIds(
      UsersShowByIdsRequest(userIds: [admin.id, user.id]),
    );
    expect(
  ...
  test("showByName", () async {
    final response = await userClient.users.showByName(
      UsersShowByUserNameRequest(userName: user.username),
    );
  ...
  test("notes", () async {
    ...
    final response = await userClient.users.notes(
      UsersNotesRequest(userId: admin.id),
    );
  ...
  test("clips", () async {
    ...
    final response = await userClient.users.clips(
      UsersClipsRequest(userId: admin.id),
    );
  ...
  test("followers", () async {
    ...
    final response = await userClient.users.followers(
      UsersFollowersRequest(userId: user.id),
    );
  ...
  test("following", () async {
    ...
    final response = await userClient.users.following(
      UsersFollowingRequest(userId: user.id),
    );
  ...
  test("reactions", () async {
    ...
    final response = await userClient.users.reactions(
      UsersReactionsRequest(userId: user.id),
    );
  ...
  group("search", () {
    test("detailed", () async {
      final response = await userClient.users.search(
        UsersSearchRequest(query: user.username),
      );
    ...
    test("lite", () async {
      final response = await userClient.users.search(
        UsersSearchRequest(query: user.username, detail: false),
      );
      expect(response.map((e) => e.id), contains(user.id));
  ...
  test("recommendation", () async {
    final response = await userClient.users.recommendation(
  ...
  test("users", () async {
    ...
    final response = await userClient.users.users(
      UsersUsersRequest(sort: UsersSortType.createdAtDescendant),
    );
  ...
  test("update-memo", () async {
    final newUser = (await adminClient.createUser()).user;
    await userClient.users.updateMemo(
      UsersUpdateMemoRequest(userId: newUser.id, memo: "test"),
    );
    final updated = await userClient.users.show(
  ...
  test("flashs", () async {
    ...
    final response = await userClient.users.flashs(
      UsersFlashsRequest(userId: user.id),
    );
    expect(response.map((e) => e.id), contains(flash.id));
  });

  test("featured-notes", () async {
    final response = await userClient.users.featuredNotes(
      UsersFeaturedNotesRequest(userId: user.id),
    );
    response.toList();
  });
  ...
  group("gallery", () {
    test("posts", () async {
      ...
      final post = await userClient.gallery.posts.create(
        GalleryPostsCreateRequest(title: "test", fileIds: [file.id]),
      );
      final response = await userClient.users.gallery.posts(
        UsersGalleryPostsRequest(userId: user.id),
      );
  ...
  group("lists", () {
    ...
    test("create-from-public", () async {
      final list = await adminClient.users.list.create(
        UsersListsCreateRequest(name: "test"),
      );
      await adminClient.users.list.update(
        UsersListsUpdateRequest(listId: list.id, isPublic: true),
      );
      await adminClient.users.list.push(
        UsersListsPushRequest(listId: list.id, userId: user.id),
      );
    ...
    test("delete", () async {
      ...
      await userClient.users.list.delete(
        UsersListsDeleteRequest(listId: list.id),
      );
    ...
    test("favorite", () async {
      ...
      await userClient.users.list.favorite(
        UsersListsFavoriteRequest(listId: list.id),
      );
      final updated = await userClient.users.list.show(
        UsersListsShowRequest(listId: list.id, forPublic: true),
      );
    ...
    test("pull", () async {
      ...
      await userClient.users.list.pull(
        UsersListsPullRequest(listId: list.id, userId: user.id),
      );
      final updated = await userClient.users.list.show(
        UsersListsShowRequest(listId: list.id),
      );
    ...
    test("show", () async {
      ...
      final response = await userClient.users.list.show(
        UsersListsShowRequest(listId: list.id),
      );
    ...
    test("unfavorite", () async {
      ...
      await userClient.users.list.unfavorite(
        UsersListsUnfavoriteRequest(listId: list.id),
      );
      final updated = await userClient.users.list.show(
        UsersListsShowRequest(listId: list.id, forPublic: true),
      );
    ...
    test("update", () async {
      ...
      final updated = await userClient.users.list.update(
        UsersListsUpdateRequest(listId: list.id, name: "updated"),
      );
