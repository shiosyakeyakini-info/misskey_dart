...
void main() async {
  ...
  group("files", () {
    ...
    test("createAsBinary", () async {
      await userClient.drive.files.createAsBinary(
    ...
    test("update", () async {
      final file = await userClient.createDriveFile();
      final updated = await userClient.drive.files.update(
        DriveFilesUpdateRequest(fileId: file.id, name: "updated"),
      );
    ...
    test("delete", () async {
      final file = await userClient.createDriveFile();
      await userClient.drive.files.delete(
        DriveFilesDeleteRequest(fileId: file.id),
      );
    ...
    test("find", () async {
      final file = await userClient.createDriveFile();
      final response = await userClient.drive.files.find(
        DriveFilesFindRequest(name: file.name),
      );
      expect(response.map((e) => e.id), contains(file.id));
    });
    ...
    test("attached-notes", () async {
      ...
      final response = await userClient.drive.files.attachedNotes(
        DriveFilesAttachedNotesRequest(fileId: file.id),
      );
    ...
    test("check-existence", () async {
      final file = await userClient.createDriveFile();
      final response = await userClient.drive.files.checkExistence(
        DriveFilesCheckExistenceRequest(md5: file.md5),
      );
    ...
    test("find-by-hash", () async {
      final file = await userClient.createDriveFile();
      final response = await userClient.drive.files.findByHash(
        DriveFilesFindByHashRequest(md5: file.md5),
      );
      expect(response.map((e) => e.id), contains(file.id));
    });
    ...
    test("show", () async {
      ...
      final response = await userClient.drive.files.show(
        DriveFilesShowRequest(fileId: file.id),
      );
  ...
  group("folders", () {
    test("folders", () async {
      ...
      final response = await userClient.drive.folders.folders(
        DriveFoldersRequest(),
      );
    ...
    test("create", () async {
      final response = await userClient.drive.folders.create(
        DriveFoldersCreateRequest(name: "test"),
      );
    ...
    test("delete", () async {
      ...
      await userClient.drive.folders.delete(
        DriveFoldersDeleteRequest(folderId: folder.id),
      );
      final folders = await userClient.drive.folders.folders(
        DriveFoldersRequest(),
      );
    ...
    test("find", () async {
      ...
      final response = await userClient.drive.folders.find(
        DriveFoldersFindRequest(name: folder.name),
      );
      expect(response.map((e) => e.id), contains(folder.id));
    });
    ...
    test("show", () async {
      ...
      final response = await userClient.drive.folders.show(
        DriveFoldersShowRequest(folderId: folder.id),
      );
