...
class MisskeyFlash {
  ...
  /// flash/my-likes
  Future<Iterable<FlashMyLikesItem>> myLikes(
      FlashMyLikesRequest request) async {
    final response =
        await _apiService.post<List>("flash/my-likes", request.toJson());
    return response
        .map((e) => FlashMyLikesItem.fromJson(e as Map<String, dynamic>));
  }

  /// flash/search
  Future<Iterable<Flash>> search(FlashSearchRequest request) async {
    final response =
        await _apiService.post<List>("flash/search", request.toJson());
    return response.map((e) => Flash.fromJson(e as Map<String, dynamic>));
  }
