...
class MisskeyAuthSession {
  ...
  Future<AuthSessionGenerateResponse> generate(
      AuthSessionGenerateRequest request) async {
    final response = await _apiService.post<Map<String, dynamic>>(
        "auth/session/generate", request.toJson());
  ...
  Future<AuthSessionShowResponse> show(AuthSessionShowRequest request) async {
    final response = await _apiService.post<Map<String, dynamic>>(
        "auth/session/show", request.toJson());
    return AuthSessionShowResponse.fromJson(response);
  }

  /// auth/session/userkey
  Future<AuthSessionUserkeyResponse> userkey(
      AuthSessionUserkeyRequest request) async {
    final response = await _apiService.post<Map<String, dynamic>>(
        "auth/session/userkey", request.toJson());
