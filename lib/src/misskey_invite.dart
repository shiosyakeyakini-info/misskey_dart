...
class MisskeyInvite {
  ...
  MisskeyInvite({required ApiService apiService})
      : _apiService = apiService;

  /// invite/create
  Future<InviteCode> create() async {
    final response = await _apiService.post<Map<String, dynamic>>("invite/create", {});
  ...
  Future<InviteLimitResponse> limit() async {
    final response = await _apiService.post<Map<String, dynamic>>("invite/limit", {});
    return InviteLimitResponse.fromJson(response);
  }

  /// invite/list
  Future<Iterable<InviteCode>> list(InviteListRequest request) async {
    final response = await _apiService.post<List>("invite/list", request.toJson());
