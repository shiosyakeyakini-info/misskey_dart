...
class MisskeyRoles {
  ...
  MisskeyRoles({required ApiService apiService})
  ...
  Future<Iterable<Note>> notes(RolesNotesRequest request) async {
    final response = await _apiService.post<List>("roles/notes", request.toJson());
    return response.map((e) => Note.fromJson(e as Map<String, dynamic>));
  }

  /// roles/show
  Future<Role> show(RolesShowRequest request) async {
    final response = await _apiService.post<Map<String, dynamic>>("roles/show", request.toJson());
    return Role.fromJson(response);
  }

  /// roles/users
  Future<Iterable<RolesUsersItem>> users(RolesUsersRequest request) async {
    final response = await _apiService.post<List>("roles/users", request.toJson());
    return response.map((e) => RolesUsersItem.fromJson(e as Map<String, dynamic>));
  }
